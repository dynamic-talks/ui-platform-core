...
export class FileConfigurationReader extends IConfigurationReader {
  ...
  readDefaultConfigFileSync(configDirPath) {
    const configData = FileConfigurationReader.readConfigFile(path.join(configDirPath, 'default'));
   ...
   */
  read({ appName, cliVars }) {
    return new Promise((resolve, reject) => {
      const configDirPath = cliVars['config-dir'];

      try {
        const configData = FileConfigurationReader.loadLocalConfigFile(path.join(configDirPath, appName));
